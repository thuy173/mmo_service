...
import {
  fetchSubProductCategory,
  updateSubProductCategory,
} from 'src/store/apps/subProductCategory/SubProductCategorySlice';
...
const UpdateSubCategory: React.FC = () => {
  ...
  const subCategory = useSelector((state: RootState) =>
    state.subProductCategories.subProductCategory.find(
  ...
  const breadcrumbs = [
    ...
    <Typography key="3" color="text.primary">
      {t('productCategory.updateSub')}
    </Typography>,
  ];

  useEffect(() => {
    if (!subCategoryId) {
      dispatch(fetchSubProductCategory(subCategoryId));
    }
  ...
  const formik = useFormik({
    initialValues: {
      ...
      orderNum: subCategory?.orderNum || 0,
      iconUrl: subCategory?.icon as File | string | undefined,
    ...
    onSubmit: async (values) => {
      ...
      updatedFormData.append('productCategoryId', values.productCategoryId.toString());
      ...
      try {
        await dispatch(updateSubProductCategory(updatedFormData));
